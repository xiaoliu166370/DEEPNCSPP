...
if __name__ == '__main__':
    for see in range(4, 50):
        ...
        aurpc = 0
        ks = 10
        kf = StratifiedKFold(n_splits=ks, random_state=see, shuffle=True)
        ...
        for index, (train, test) in enumerate(kf.split(X, y)):
            ...
            for epoch in range(150):
                ...
                if score_ > score_best:
                    ...
                    spe_best = spe_
                    AUPR_best = aupr_
                    torch.save(fakes,f'cnn_seed_{see}_fakes{index+1}.pkl')
                    torch.save(fakes_p,f'cnn_seed_{see}_fakes_p{index+1}.pkl')
                    torch.save(yts,f'cnn_seed_{see}_yts{index+1}.pkl')
            acc += acc_best
            spe += spe_best
            aurpc += AUPR_best
            f1 += f1_best
        ...
        print(f'seed {see} 10 cross validation')
